...
highwayPatrol =
    { defaultSponsor
        ...
        , perks =
            [ TeamPerk "Hot Pursuit"
                "Before the first Gear Phase of the game, after deployment, this team must nominate one enemy vehicle as the \"bogey\". If the bogey is Wrecked or disqualified, immediately nominate another enemy vehicle to be the bogey."
            , TeamPerk "Bogey at 12 o'Clock"
                "At the end of this Movement Step, if the bogey in is this vehicle's front Arc of Fire, and further than Double Range away, and can be seen by this vehicle, this vehicle may immediately resolve another Movement Step."
            , TeamPerk "Siren"
                "At the end of this vehicle's Attack Step, if this vehicle is in the bogey's rear Arc of Fire (regardless of range), the bogey must either reduce its Gear by 1 or gain 2 Hazard Tokens."
            , TeamPerk "Steel Justice"
...
verney =
    { defaultSponsor
        | name = "Verney"
        , description = "Many have taken the bent deal offered by Warden Cadeila but only one has ever earned their freedom. As skilled an engineer as he is a driver, the newly-freed Verney now specialised in building unique Frankenstein's monsters of vehicles for anyone who can afford his high-quality customs."
        , perks =
            [ TeamPerk "Trunk of Junk"
...
maxxine =
    { defaultSponsor
        ...
        , perks =
            ...
            , TeamPerk "Maxximum Drift"
                "If this vehicle resolves two Slide results in a single Movement Step, it may use the Medium Straight in place of the slide template. If this vehicle resolves three or more Slide results in a single Movement Step, it may use the Long Straight in place of the Slide template."
            , TeamPerk "Meshuggah"
                "When this vehicle resolves a Slide or Spin that ends within Medium range of a friendly vehicle without causing a Collision, this team gains 1 Audience Vote."
            ]
...
orderOfTheInferno =
    { defaultSponsor
        ...
        , perks =
            [ TeamPerk "Fire Walk With Me"
                "When this vehicle would receive damage from any weapon or effect with the Fire rule, this vehicle may reduce the damage received by up to three, to a minimum of one."
            , TeamPerk "Burning Man"
                "If this vehicle is On Fire it gains +1 to all Evade dice."
            , TeamPerk "Cult of Flame"
...
beverly =
    { defaultSponsor
        ...
        , perks =
            [ TeamPerk "Graveyard Shift"
                "At the start of the game, after deployment, all vehicles in this team except one must gain the Ghost Rider special rule."
            , TeamPerk "Ghost Rider"
            ...
            , TeamPerk "At the Crossroads"
                "This team may choose to pay only 1 Audience Vote to respawn a vehicle. If they do, the respawned vehicle must gain the Ghost Rider special rule."
            , TeamPerk "Inexorable"
...
rustysBootleggers =
    { defaultSponsor
        ...
        , perks =
            [ TeamPerk "Party Hard"
                "At the end of this vehicle's Attack Step, if this vehicle has more Hazard Tokens than the sum of the Hazard Tokens on all other enemy vehicles within Medium range combined, this vehicle's controller gains 1 Audience Vote for each enemy vehicle within Medium range of this vehicle."
            , TeamPerk "Dutch Courage"
                "Vehicles in this team only Wipeput when they have 8 Hazard Tokens."
            , TeamPerk "As Straight as I'm Able"
                "This vehicle does not gain a Hazard Token from the articulated rule if it selects a template that is not a Straight."
            , TeamPerk "Over the Limit"
            ...
            , TeamPerk "Trailer Trash"
                "This team may purchase Trailers. This team must contain either: one or more Medium or Heavyweight vehicles equipped with a trailer upgrade, or a War Rig."
            , TeamPerk "Haulage"
                "Each vehicle on this team equipped with a trailer upgrade, and • each War Rig on this team, may equip a single trailer cargo upgrade for free. "
            ]
...
badass =
    ...
    , VehiclePerk "Bullet-Time" 3 "If this vehicle resolves a Slide result during its Movement Step, this may select one of its weapons to count as Turret-mounted for the rest of the activation."
    ]
...
built =
    ...
    , VehiclePerk "Splashback" 5 "Once per step, when this vehicle loses one or more Hull Points, make a 1D6 attack against each vehicle within Medium range at end of that step."
...
daring =
    ...
    , VehiclePerk "Handbrake Artist" 3 "When applying a Spin result, this vehicle may choose to face any direction."
...
military =
    [ VehiclePerk "Dead-Eye" 2 "During this vehicle's attack step, up to three crewmembers in this vehicle may gain a +1 bonus to hit if making a shooting attack at a target within double range and not within medium range. Critical hits still occur only on the natural roll of a 6."
...
precision =
    [ VehiclePerk "Mister Fahrenheit" 2 "This vehicle cannot gain more than 2 Hazard Tokens from Collisions during a single activation."
    , VehiclePerk "Moment of Glory" 2 "Once per game, after rolling the Skid Dice, but before resolving the results, this vehicle may immediately change any number of Skid Dice to any results they choose."
    ...
    , VehiclePerk "Trick Driving" 3 "This vehicle may select a movement template as if its current Gear was one higher or one lower."
    , VehiclePerk "Easy Rider" 5 "Once per turn, this vehicle may discard one rolled skid die result before applying the results."
    ]
...
speed =
    ...
    , VehiclePerk "Downshift" 3 "At the end of a Movement Step in which this vehicle changed down one or more Gears, this vehicle may make a forced move Short Straight forward."
...
technology =
    ...
    , VehiclePerk "Satellite Navigation" 2 "When this vehicle resolves its Skid Dice, this vehicle's controller may set aside one Shift result. This vehicle may have any number of Shift results set aside. Any vehicle in this team may use these set aside Shift results during a later Movement Step, as if they rolled them in that Movement Step."
    , VehiclePerk "Mobile Mechanic" 3 "Once per turn, during the attack step, a single crewmember may be assigned to perform a FIELD REPAIR instead of attack with a weapon. This vehicle gains 1 hull point, which may not take the remaining hull points above the vehicle's hull value."
    , VehiclePerk "Eureka!" 4 "Once per game, at the start of its Attack Step, this vehicle's controller may declare any weapon that this vehicle has not attacked with yet this game. This vehicle counts as being armed with the declared weapon, on a facing of their choice, for the next attack only."
    ]
